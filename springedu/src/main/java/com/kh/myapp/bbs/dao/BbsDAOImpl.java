...
public class BbsDAOImpl implements BbsDAO {
	...
	public List<BbsDTO> list() throws Exception {
		...
		list = (List<BbsDTO>)jdbcTemplate.query(sql.toString(),new BeanPropertyRowMapper(BbsDTO.class));
		return list;
	}
	...
	public BbsDTO view(String bnum) throws Exception {
		...
		bbsdto = jdbcTemplate.queryForObject(sql.toString(), new Object[] {bnum} ,new BeanPropertyRowMapper<BbsDTO>(BbsDTO.class) );
		//조회수 증가
...
//		});
	...
	private boolean isReply(String bnum) {
		...
		// sql query의 결과를 가져올 때 - count(bnum)을 가져와야함
		cnt = jdbcTemplate.queryForObject(sql.toString(), new Object[] {bnum, bnum}, Integer.class);
	...
	private int updateStep(int bgroup, int bstep) {
		...
		if(cnt > 0) {
			logger.info("게시글 그룹/ 게시글 스탭 1증"  + cnt);
		} else {
			logger.info("게시글 그룹/ 게시글 스탭 1증"  + cnt);
		}
